...
(() => {
  ...
  const COLORS = {
    blue: '#3b6ff5', cyan: '#18a7c9', violet: '#7357e8', green: '#24a779',
    blueSoft: 'rgba(59,111,245,.18)', cyanSoft: 'rgba(24,167,201,.18)',
    violetSoft: 'rgba(115,87,232,.18)', greenSoft: 'rgba(36,167,121,.18)',
    grid: '#e9edf4', text: '#5d687f'
  };

  const state = { rows: [], charts: {}, initializedFilters: false };
  ...
  function aggregate(rows) {
    const sums = rows.reduce((acc, row) => {
      ...
      } else if (row.scored > row.completed) {
        acc.invalidPerformance += 1;
      }
      return acc;
    }, { total: 0, completed: 0, scored: 0, performanceCompleted: 0, performanceScored: 0, invalidPerformance: 0 });
    return {
      ...sums,
      streams: rows.length,
    ...
    return [...groups.entries()]
      .map(([key, grouped]) => {
        ...
        return {
          ...
          label: granularity === 'month' ? `${MONTHS[first.month - 1]} ${String(first.year).slice(-2)}` : `Q${first.quarter} ${first.year}`,
          ...value
        };
      })
  ...
  function syncFilters() {
    ...
    [...new Set(state.rows.map((r) => r.course))].sort((a, b) => a.localeCompare(b, 'ru')).forEach((course) => option(courseSelect, course, course));
    [...new Set(state.rows.map((r) => r.year).filter(Boolean))].sort((a, b) => b - a).forEach((year) => option(yearSelect, String(year), String(year)));
  ...
  function rateClass(value) {
    ...
    if (value >= .8) return 'rate-good';
    if (value >= .55) return 'rate-mid';
    return 'rate-low';
  }
  ...
  function renderKpis(rows) {
    ...
    el('kpiCompletedShare').textContent = kpi.total ? `${fmtPct.format(kpi.completed / kpi.total)} от всех студентов` : 'Нет данных';
  ...
  function baseOptions(percent = false) {
    return {
      ...
      plugins: {
        ...
        tooltip: { backgroundColor: '#172033', padding: 11, cornerRadius: 9, callbacks: percent ? { label: (ctx) => `${ctx.dataset.label}: ${fmtPct.format(ctx.raw / 100)}` } : {} }
      },
      scales: {
  ...
  function renderCharts(rows) {
    ...
    updateChart('rates', 'rateChart', {
      ...
      data: {
        labels,
        datasets: [
          { label: 'CoR', data: groups.map((g) => g.cor === null ? null : g.cor * 100), borderColor: COLORS.blue, backgroundColor: COLORS.blueSoft, pointBackgroundColor: COLORS.blue, pointRadius: 3, pointHoverRadius: 5, tension: .28, spanGaps: true },
          { label: 'Успеваемость', data: groups.map((g) => g.performance === null ? null : g.performance * 100), borderColor: COLORS.green, backgroundColor: COLORS.greenSoft, pointBackgroundColor: COLORS.green, pointRadius: 3, pointHoverRadius: 5, tension: .28, spanGaps: true }
        ]
      },
    ...
    updateChart('students', 'studentChart', {
      ...
      data: {
        labels,
        datasets: [
          { label: 'Всего студентов', data: groups.map((g) => g.total), backgroundColor: COLORS.blueSoft, borderColor: COLORS.blue, borderWidth: 1, borderRadius: 6 },
          { label: 'Окончили', data: groups.map((g) => g.completed), backgroundColor: COLORS.cyanSoft, borderColor: COLORS.cyan, borderWidth: 1, borderRadius: 6 },
          { label: 'Набрали >50%', data: groups.map((g) => g.scored), backgroundColor: COLORS.violetSoft, borderColor: COLORS.violet, borderWidth: 1, borderRadius: 6 }
        ]
      },
    ...
    const courseData = [...byCourse.entries()]
      .map(([course, values]) => ({ course, ...aggregate(values) }))
      .sort((a, b) => b.total - a.total);

    updateChart('courses', 'courseChart', {
      type: 'bar',
      data: {
        labels: courseData.map((x) => x.course),
        datasets: [
          { label: 'CoR', data: courseData.map((x) => x.cor === null ? null : x.cor * 100), backgroundColor: COLORS.blue, borderRadius: 6 },
          { label: 'Успеваемость', data: courseData.map((x) => x.performance === null ? null : x.performance * 100), backgroundColor: COLORS.green, borderRadius: 6 }
        ]
      },
      options: {
        ...baseOptions(true),
        indexAxis: 'y',
        scales: {
          x: { min: 0, suggestedMax: 100, grid: { color: COLORS.grid }, ticks: { callback: (v) => `${v}%` } },
  ...
  function periodText(row) {
    if (!row.year || !row.quarter) return 'Без периода';
  ...
  function renderTable(rows) {
    const sorted = [...rows].sort((a, b) => {
      ...
      return dateB - dateA || a.course.localeCompare(b.course, 'ru');
    });
  ...
  function renderStatus(rows) {
    const noPeriod = rows.filter((r) => !r.year || !r.quarter).length;
    const notFinished = rows.filter((r) => r.completed === 0 && r.total > 0).length;
    const invalidPerformance = rows.filter((r) => r.scored > r.completed).length;
    const messages = [];
    if (noPeriod) messages.push(`${noPeriod} ${plural(noPeriod, ['строка не участвует', 'строки не участвуют', 'строк не участвуют'])} в графиках по периодам: не заполнены год или квартал.`);
    if (notFinished) messages.push(`${notFinished} ${plural(notFinished, ['поток ещё не имеет', 'потока ещё не имеют', 'потоков ещё не имеют'])} окончивших студентов; процентные показатели для них не рассчитываются.`);
    if (invalidPerformance) messages.push(`${invalidPerformance} ${plural(invalidPerformance, ['строка исключена', 'строки исключены', 'строк исключены'])} из расчёта успеваемости: число набравших >50% превышает число окончивших.`);
    el('statusPanel').textContent = messages.join(' ');
    el('statusPanel').classList.toggle('hidden', messages.length === 0);
  }

  function render() {
    const rows = getFilteredRows();
    const hasRows = rows.length > 0;
    document.querySelector('.dashboard').classList.toggle('hidden', !hasRows);
    el('emptyState').classList.toggle('hidden', hasRows);
    if (!hasRows) return;

    const course = el('courseFilter').value;
    el('subtitle').textContent = course === 'all'
      ? 'Квартальная динамика завершения обучения и успеваемости'
      : `Курс ${course}: динамика завершения обучения и успеваемости`;
    renderKpis(rows);
    renderCharts(rows);
    renderTable(rows);
    renderStatus(rows);
  ...
  function bindEvents() {
    ['courseFilter', 'yearFilter', 'quarterFilter', 'granularityFilter'].forEach((id) => el(id).addEventListener('change', render));
  ...
  function setRows(rows) {
    state.rows = rows.map(normalize).filter((r) => r.course);
    syncFilters();
    render();
  }
  ...
  function setupGrist() {
    ...
    grist.onRecords((records) => {
      ...
      if (!mapped) {
        ...
        el('emptyState').querySelector('p').textContent = 'Сопоставьте все обязательные колонки в панели настройки виджета Grist.';
        return;
      }
      setRows(mapped);
    });
    ...
    for (let year = 2024; year <= 2025; year++) {
      for (let quarter = 1; quarter <= 4; quarter++) {
        courses.forEach((course, ci) => {
          ...
          rows.push({ id: id++, course, stream: `${year}-${quarter}-${ci + 1}`, month: quarter * 3, quarter, year, total, completed, scored });
        });
      }
    }
    return rows;
